...
def test_async_status_extras(tmp_path: pathlib.Path) -> None:
    f = tmp_path / "a"
    f.touch()

    async def go() -> None:
        p = AsyncPath(f)
        assert await p.is_symlink() is False
        assert (await p.lstat()).st_size == 0
        assert await p.samefile(f) is True

    _run(go())


def test_async_copy_into_move_into(tmp_path: pathlib.Path) -> None:
    async def go() -> None:
        src = AsyncPath(tmp_path) / "a.txt"
        await src.write_text("x")
        await (AsyncPath(tmp_path) / "dest").mkdir()
        await src.copy_into(tmp_path / "dest")
        assert (tmp_path / "dest" / "a.txt").read_text() == "x"

        src2 = AsyncPath(tmp_path) / "c.txt"
        await src2.write_text("y")
        await (AsyncPath(tmp_path) / "dest2").mkdir()
        await src2.move_into(tmp_path / "dest2")
        assert (tmp_path / "dest2" / "c.txt").read_text() == "y"

    _run(go())


def test_async_absolute_expanduser_rename_replace(
    tmp_path: pathlib.Path,
) -> None:
    async def go() -> None:
        p = AsyncPath(tmp_path) / "a"
        assert isinstance(await p.absolute(), AsyncPath)
        assert isinstance(await AsyncPath("~/x").expanduser(), AsyncPath)
        f = AsyncPath(tmp_path) / "r.txt"
        await f.write_text("x")
        g = await f.rename(tmp_path / "r2.txt")
        assert isinstance(g, AsyncPath)
        h = AsyncPath(tmp_path) / "r3.txt"
        await h.write_text("y")
        await h.replace(tmp_path / "r2.txt")
        assert (tmp_path / "r2.txt").read_text() == "y"

    _run(go())


def test_async_file_readline_and_flush(tmp_path: pathlib.Path) -> None:
    async def go() -> None:
        p = AsyncPath(tmp_path) / "a.txt"
        async with await p.open("w") as handle:
            ...
            await handle.flush()
        async with await p.open() as handle:
            assert await handle.readline() == "l1\n"

    _run(go())


class _SyncProbeAsyncMethod(os.PathLike):
    """Passes the async-driver probe but returns a coroutine at call time."""
    ...
    def exists(self) -> bool:  # sync -> probe passes
        return True

    def read_bytes(self) -> object:  # returns an un-awaited coroutine
        async def _c() -> bytes:
            return b""
...
def test_bridge_rejects_a_coroutine_result() -> None:
    async def go() -> None:
        p = AsyncPath(_SyncProbeAsyncMethod())
        with pytest.raises(UnsupportedPathOperation):
            await p.read_bytes()
