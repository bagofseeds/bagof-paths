...
# -- coverage for adapter branches -----------------------------------------
class _NoReplace(os.PathLike):
    """A local driver with no replace(): move falls back to copy + unlink."""
...
class _ExdevReplace(_NoReplace):
    """A local driver whose replace() reports a cross-device move (EXDEV)."""

    def replace(self, target: object) -> None:
        import errno

        raise OSError(errno.EXDEV, "cross-device")


def test_copy_missing_source_raises(tmp_path: pathlib.Path) -> None:
    with pytest.raises(FileNotFoundError):
        (Path(tmp_path) / "nope").copy(tmp_path / "x")


def test_move_without_replace_uses_copy_delete(tmp_path: pathlib.Path) -> None:
    src = tmp_path / "a.txt"
    src.write_text("x")
    Path(_NoReplace(src)).move(tmp_path / "b.txt")
    assert (tmp_path / "b.txt").read_text() == "x"
    assert not src.exists()


def test_move_cross_device_falls_back(tmp_path: pathlib.Path) -> None:
    src = tmp_path / "a.txt"
    src.write_text("x")
    Path(_ExdevReplace(src)).move(tmp_path / "b.txt")
    assert (tmp_path / "b.txt").read_text() == "x"
    assert not src.exists()


def test_copy_to_non_local_target_raises(tmp_path: pathlib.Path) -> None:
...
def test_upath_copy_move_walk() -> None:
    UPath = pytest.importorskip("upath").UPath
    base = UPath("memory://bagofpaths-cov/dir")
    base.mkdir(parents=True, exist_ok=True)
    (base / "a.txt").write_text("x")
    Path(base / "a.txt").copy(base / "b.txt")
    assert (base / "b.txt").read_text() == "x"
    Path(base / "b.txt").move(base / "c.txt")
    assert (base / "c.txt").read_text() == "x"
    rows = list(Path(base).walk())
    assert any(fs for _d, _dn, fs in rows)


def test_cloudpathlib_copy_move() -> None:
    local = pytest.importorskip("cloudpathlib.local")
    client = local.LocalS3Client()
    base = local.LocalS3Path("s3://bagofpaths-cov/dir", client=client)
    (base / "a.txt").write_text("x")
    Path(base / "a.txt").copy(base / "b.txt")
    assert (base / "b.txt").read_text() == "x"
    Path(base / "b.txt").move(base / "c.txt")
...
def test_walk_bottom_up_and_on_error(tmp_path: pathlib.Path) -> None:
    root = Path(tmp_path)
    sub = root / "sub"
    sub.mkdir()
    (sub / "f.txt").touch()
    order = [str(dp) for dp, _dn, _fn in root.walk(top_down=False)]
    assert order[-1] == str(root)  # root yielded last, bottom-up
    errors = []
